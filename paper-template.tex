...
%------------------------------------------------------------------------------
%  Derived from the manuscript styling of
%      J. Lucas Reddinger, "Temptation: Immediacy and certainty".
%
%  It reproduces that paper's preamble, custom commands, title page, and
%  online-supplement machinery, but with generic placeholder content so you
%  can drop in a new paper. Search for "TODO" to find what to replace.
%
...
%      pdflatex paper-template
%
%  By default this file uses Minion Pro (a commercial font), matching the
%  source paper. If it is not installed, switch to the free Charter stack in
%  the "Font face packages" block below and it builds on any TeX Live / MiKTeX.
%==============================================================================
...
%%%% Font face packages -------------------------------------------------------
%
%  Three interchangeable stacks. EXACTLY ONE must be active (uncommented).
%  Stack 1 (Minion Pro) is the default, matching the source paper.
%
%  The %%arxiv-font%% / %%not-arxiv-font%% tags let a build script swap stacks
%  automatically (e.g. the free stack 2 for an arXiv upload, Minion Pro for the
%  camera-ready). Keep them if you use such a script; otherwise ignore.

% 1. Minion Pro + Source Sans Pro + Inconsolata   <-- DEFAULT
%    Minion Pro is a commercial serif; it must be installed (it is, in the
%    environment this template came from). Switch to stack 2 for a free build.
...
% 2. Charter + Source Sans Pro + Inconsolata   (free fallback, e.g. for arXiv)
%\usepackage[charter]{mathdesign}                        % math  %%arxiv-font%%
%\usepackage[sups,osf]{XCharter}                         % serif %%arxiv-font%%
%\usepackage[semibold]{sourcesanspro}                    % sans  %%arxiv-font%%
%\usepackage[scaled=1.04,varqu,varl]{inconsolata}        % mono  %%arxiv-font%%

% 3. TeX Gyre Pagella (Palatino) + TeX Gyre Heros + Inconsolata + newtx math
...
\usepackage{booktabs}
\usepackage{ntheorem}
% Warning: uniquelist=false would violate the Chicago Manual of Style.
% CMOS opposes use of an ibidem mechanism, hence noibid.
...
% Minion Pro's math lacks a blackboard "1", so this pulls the glyph from the
% doublestroke font (requires the dsserif package, which is installed).
\newcommand{\mybb}[1]{{\text{{\usefont{U}{DSSerif}{m}{n}#1}}}}
% Free-font fallback if you switch stacks: \usepackage{dsfont} above, then use
%\newcommand{\mybb}[1]{\mathds{#1}}
...
\newcommand{\papertitle}{The paper title: An optional subtitle}  % TODO (sentence case)
\newcommand{\paperauthor}{First M.~Last}                         % TODO
\newcommand{\paperdate}{20 June 2026}                           % day month year
...
\textbf{\sffamily\LARGE \papertitle\symfootnote{TODO acknowledgements: thank
colleagues, seminar audiences, funders, and IRB/registration details here.
Cite with the usual commands, e.g.\ \textcite{RoeSmith2019} provide data and
code.}}
...
\paperauthor\symfootnote{TODO affiliation: Department, University, mailing
address; \href{mailto:you@example.edu}{you@example.edu}.}
...
\paragraph{Abstract}
TODO. One paragraph stating the question, method, and headline finding. Keep it
self-contained: a reader should grasp the contribution without the body. End
with the takeaway or implication.
...
\paragraph{\emph{JEL} Codes} C91, D80, D90 % TODO

\paragraph{Keywords} keyword one, keyword two, keyword three, keyword four % TODO
...
% Switch the main text to 1.5 line spacing. \setstretch sets the spacing from
% here on (within the document) without opening a group, unlike the bare
% \spacing{} command, which the source paper used.
\setstretch{1.5}

\section{Introduction}\label{sec:intro}

TODO. Open with the motivating question and why it matters. This template
preserves the source paper's formatting machinery; the prose below only
...
TODO. State the framework. The indicator macro \verb|\mybb{1}(...)| typesets a
blackboard ``1'', e.g.\ $\mybb{1}(x>0)$. A displayed, labelled equation:%
\begin{align}
...
TODO. The primary testable prediction, e.g.\ $\beta_{\text{treat}} <
\beta_{\text{control}}$.
\end{hypothesis}

\begin{hypothesis}\label{hyp:secondary}
TODO. A secondary prediction.
\end{hypothesis}
...
\begin{enumerate}[label=\textbf{\sffamily Step \arabic*:}, align=left, leftmargin=*]
    \item TODO. Describe the first step.
    \item TODO. Describe the second step. To attribute a method to two authors
    with an en-dash, use \textcitedash{RoeSmith2019} standard errors.
\end{enumerate}
...
\begin{table}\centering
  \begin{threeparttable}
    ...
    \begin{tablenotes}\footnotesize
      \emph{Note:} TODO. Numbers in the \texttt{d\{1.3\}} columns align on the
      decimal point; wrap headers and integer cells in \verb|\multicolumn{1}{c}{...}|.
    \end{tablenotes}
  \end{threeparttable}
\end{table}

% ---- Figure: two TikZ panels via subfig (self-contained, no external files) -
\begin{figure}\centering
  \subfloat[First panel\label{fig:example-a}]{%
    \begin{tikzpicture}
      \draw[thick] (0,0) rectangle (4,3);
      \node at (2,1.5) {panel A placeholder};
    \end{tikzpicture}}
  \hfill
  \subfloat[Second panel\label{fig:example-b}]{%
    \begin{tikzpicture}
      \draw[thick] (0,0) rectangle (4,3);
      \node at (2,1.5) {panel B placeholder};
    \end{tikzpicture}}
  \caption{Example figure with two panels\label{fig:example}}
\end{figure}

% ---- Dropping in externally generated assets (R / Stata / Python) ----------
% Mirror the source paper: \input a booktabs/threeparttable .tex for tables and
...
TODO. Restate the contribution, note limitations, and point to implications and
future work.
...
\vspace{9pt}
{\sffamily\bfseries\large Supplement for online publication}\\
\vspace{18pt}
{\large \paperauthor}\\
\vspace{9pt}
{\large \paperdate}\\
...
TODO. Supplement sections are numbered S.1, S.2, \ldots\ and appear in the
contents above. Reference them like \cref{sec:supp:first} or
\cref{sec:supp:second}.
...
\begin{figure}[h]\centering
  \begin{tikzpicture}
    ...
    \node at (3,1.5) {supplement figure placeholder};
  \end{tikzpicture}
  \caption{A supplementary figure\label{fig:supp-example}}
\end{figure}
